  ...
  const io = new IntersectionObserver((entries) => {
    entries.forEach(_e => {
      if (_e.isIntersecting) {
        _e.target.classList.add('show')
        io.unobserve(_e.target)
      }
    })
  }, { threshold: 0.2 })
...
(() => {
  ...
  if (typeof window.QRCode === 'function') {
    ...
    try {
      // instantiate QRCode and keep reference; avoid using 'new' purely for side-effects
      const _qr = new window.QRCode(qrEl, {
        text: url,
        width: size,
        height: size,
        colorDark: '#000000',
      ...
      // _qr can be used later if needed
    } catch (_) { /* keep placeholder if fails */ }
  }
})()

// Small helper to force reflow without 'void'
function reflow (el) {
  if (!el) return
  // read a layout property to trigger reflow
